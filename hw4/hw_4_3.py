...
for mentor in mentors:
    ...
    # Допишите ниже код, который добавляет списки имён
    # в общий список mentors_names:
    mentors_names.append(course_names)

# Храните здесь пары курсов, в которых есть совпавшие имена
pairs = []
# Попарное сравнение "наборов" преподавателей на курсах.
# Каждую новую пару запоминаем для исключения повторов.
for id1 in range(len(mentors_names)):
    for id2 in range(len(mentors_names)):
        ...
        if id1 == id2:
            ...
            # Допишите ниже код для сравнения двух "наборов" преподавателей.
            # Подсказка: используйте множества
            intersection_set = set(mentors_names[id1])\
                            & set(mentors_names[id2])
            # Допишите проверку, что результат не пустой, имена есть
            if len(intersection_set) > 0:
                # Допишите ниже код, который проверяет,
                # что эта пара ещё не встречалась
                pair = {courses[id1], courses[id2]}
                # Если pair еще не встречалась, то выведите на экран два курса
                ...
                if pair not in pairs:
                    ...
                    # Можете использовать string.join()
                    print(f'На курсах \'{courses[id1]}\' и \'{courses[id2]}\''
