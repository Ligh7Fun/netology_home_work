...
for title, mentors_, duration_ in zip(courses, mentors, durations):
    course_dict = {'title': title, 'mentors': mentors_, 'duration': duration_}
...
# Как видите, в duration встречаются одинаковые длительности курса.
# Допишите код, который это учитывает
# Подсказка 1: найдите индексы, по которым в списке durations встречается
...
for index, _ in enumerate(durations):
    if max_ == durations[index]:
        maxes.append(index)
    elif min_ == durations[index]:
        minis.append(index)

# Соберите все названия самых коротких и самых длинных курсов
# Так как курсов с одной длительностью может быть больше одного,
# создайте список названий самых коротких (courses_min)
# и самых длинных (courses_max) курсов
courses_min = []
courses_max = []
for id in minis:
    # Допишите код, который берёт по id нужный курс из courses_list
    # и получает название курса из ключа "title"
...
print(f'Самый длинный курс(ы): {", ".join(courses_max)} - {max_} месяца(ев)')
